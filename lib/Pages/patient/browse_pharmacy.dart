...
class BrowsePharmacy extends StatefulWidget {
  const BrowsePharmacy({Key? key}) : super(key: key);

  @override
  _BrowsePharmacyState createState() => _BrowsePharmacyState();
}

class _BrowsePharmacyState extends State<BrowsePharmacy>
  ...
  Future<void> _getNearbyPharmacy() async {
    // Get the current location
    final location = await _getCurrentLocation();
    final request = Location(location.latitude, location.longitude);

    // Set up the nearby search request

    // Send the nearby search request
    final response =
        await _places.searchNearbyWithRadius(request, 5000, type: "Pharmacy");

    // Add the hospital markers to the map
    setState(() {
      _markers.addAll(response.results.map((result) => Marker(
  ...
  final _places =
      GoogleMapsPlaces(apiKey: "AIzaSyChELQf8W69lDdoq4sH733q8f0SHR49DNI");
